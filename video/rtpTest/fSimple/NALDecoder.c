...
//NALU(Network Abstract Layer Unit)：两标准中的比特流都是以NAL为单位，每个NAL单元包含一个RBSP，
//NALU的头信息定义了RBSP所属类型。类型一般包括序列参数集（SPS）、图像参数集（PPS）、增强信息（SEI）、条带（Slice）等
typedef struct
{
...
int main(int argc, char* argv[])
{
	...
	if(argc > 1)
	{
		OpenBitstreamFile(argv[1]);	//打开264文件，并将文件指针赋给bits,在此修改文件名实现打开别的264文件。
	}
	else
	{
	...
	while(!feof(bits)) 
	{
		...
		memset(sendbuf,0,sizeof(sendbuf));	//清空sendbuf；此时会将上次的时间戳清空，因此需要ts_current来保存上次的时间戳值
