...
int mark()
{
		...
		fmt.fmt.pix.width=320;//宽，必须是16的倍数 320 640
		fmt.fmt.pix.height=240;//高，必须是16的倍数 240 480
...
void *pthread_video(void *arg)
{
		...
		//char bufs[1024*64];
		video_on();
		databuf=(buf_t *)malloc(sizeof(buf_t)+buffers[0].length); //bufs;
		if((fps = fopen("test.yuv","wb")) == NULL)
		{
