...
int setMark(void)
{
    ...
    fmt.fmt.pix.width = VIDEO_W;//宽，必须是16的倍数
    fmt.fmt.pix.height = VIDEO_H;//高，必须是16的倍数
    fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_YUYV;//=V4L2_PIX_FMT_YUYV;// 视频数据存储类型
...
void *pthread_encode(void *arg)
{
	...
	clock_t clockStart = clock();
        if(YUV422To420(databuf->buf, tbuffers, VIDEO_W, VIDEO_H) != 0)//jpg2yuv(databuf->buf, databuf->datasize, tbuffers) != 0)
	{
        ...
        { 
            ...
            if(encode_one_frame(tbuffers, ptrSendDataBuf, &encodeDatalen) >=0)
            {
